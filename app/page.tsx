import { Post } from "@/shared/domains/posts/models/post";
...
const postEndpoint = '/contents/NiltJs';
...
async function getLastPost() {
  ...
  posts.sort(
    (a,b) =>
       (b.created_at as unknown as number) - 
        (a.created_at as unknown as number)
  );

  const [lastPostFromList] = posts;

  const lastPostResponse = await fetch(`${baseURL}${postEndpoint}/${lastPostFromList.slug}`);
        ...
        <div className={`
         w-[calc(100%+5rem)] h-[5.5rem]
          ...
          <div className="flex items-center justify-end mt-9 mb-4">
            ...
            <Text variant="xs" className=" text-blue-400 hover:text-blue-600 transition-all duration-200 ease-in-out">Ver Outros Posts</Text> </Link>
            <Link href={`/blog/${lastPost.slug}`} className="py-2 px-3 bg-gray-100 hover:bg-gray-200 transition-all duration-200 ease-in-out rounded">
            <Text variant="xs" className="font-bold text-gray-800">Continuar Lendo</Text></Link>
